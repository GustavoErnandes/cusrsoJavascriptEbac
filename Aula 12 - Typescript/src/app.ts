...
class AppState {
    ...
    addItem(
        ...
        const todoItem = new ItemStructure(id, input, description, status);
...
class TodoItem {
    ...
    private display() {
      ...
      this.ulElement.querySelector('h3')!.textContent = this.description;
  ...
  class TodoList {
    ...
    private display() {
      ...
      for (let todo of this.todoItems) {
        new TodoItem(todo.id, todo.input, todo.description, this.todoItems);
      }
    }
  ...
  class TodoInput {
    ...
    constructor() {
      ...
      this.todoDescription = document.querySelector('description')! as HTMLInputElement;
    ...
    private submitHandler(e: Event) {
      ...
      const getTodoDescription =  this.todoDescription.value;
      const ValidatedText = this.validation(getTodoValue);
      const ValidatedDescription = this.validation(getTodoDescription);
      if (ValidatedText && ValidatedDescription) {
        const id = Math.random().toString();
        appState.addItem(id, ValidatedText, ValidatedDescription, STATUS.Pending);
        this.clearFormInput();
      } else {
        this.clearFormInput();
      }
